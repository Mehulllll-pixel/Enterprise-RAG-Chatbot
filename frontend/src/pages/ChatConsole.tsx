...
export const ChatConsole: React.FC = () => {
  ...
  const fetchSessions = async () => {
    try {
      ...
      if (response.length > 0 && !activeSessionId) {
  ...
  // Helper colors for confidence score
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] relative overflow-hidden bg-darkBg">
      
      {/* 1. Sidebar Nav Conversations list */}
      <div className={`absolute md:relative inset-y-0 left-0 w-80 bg-darkSurface border-r border-gray-900/60 z-20 transition-transform duration-300 md:translate-x-0 flex flex-col ${
        sidebarOpen ? 'translate-x-0' : '-translate-x-full'
      }`}>
        <div className="p-4 border-b border-gray-900/60">
          <button
            onClick={handleNewChat}
            className="w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-brand-500 hover:bg-brand-600 text-white font-medium rounded-xl transition-all shadow-md text-sm"
          >
            ...
            sessions.map((session) => (
              ...
              >
                ...
                <button
                  ...
                  className="p-1 hover:bg-red-500/20 text-gray-500 hover:text-red-400 rounded-lg transition-opacity md:opacity-0 group-hover:opacity-100"
                >
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-darkBg relative">
        {/* Mobile Header Bar */}
        <div className="md:hidden flex items-center justify-between p-3 border-b border-gray-900/60 bg-darkSurface">
          <button
        ...
        {!activeSessionId ? (
          <div className="flex-1 flex items-center justify-center p-8 text-center">
            <div className="max-w-md space-y-6">
              ...
              <button
                onClick={handleNewChat}
                className="px-6 py-3 bg-brand-500 hover:bg-brand-600 text-white font-medium rounded-xl transition-all shadow-lg text-sm inline-flex items-center gap-2"
              >
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
              {messages.length === 0 && !streamingResponse && (
                <div className="flex flex-col items-center justify-center h-full text-center text-gray-500 p-8">
                  ...
                  <p className="font-semibold text-gray-400">Secure pipeline connected</p>
              ...
              {messages.map((msg, index) => (
                <div 
                  ...
                  className={`flex gap-4 p-4 rounded-2xl max-w-3xl border ${
                    msg.role === 'user'
                      ? 'ml-auto bg-darkCard border-gray-800/80 text-white flex-row-reverse'
                      : 'bg-darkSurface/50 border-gray-900/60 text-gray-200'
                  }`}
                >
                  {/* Icon */}
                  ...
                  <div className="space-y-3 min-w-0 flex-1">
                      ...
                      <div className="flex flex-wrap items-center gap-3 pt-2 text-[10px] text-gray-500">
                        ...
                        {msg.latency_ms !== undefined && (
                          <span className="flex items-center gap-1">
                            <Clock className="h-3 w-3 text-gray-650" />
                        ...
                        {msg.confidence_score !== undefined && (
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border ${getConfidenceColorClass(msg.confidence_score)}`}>
                            <BarChart3 className="h-3 w-3" />
                            <span>Confidence: {Math.round(msg.confidence_score * 100)}%</span>
                          </span>
                        )}

                        {/* Citations Drawer trigger */}
                        {msg.citations && msg.citations.length > 0 && (
                          <button
                            onClick={() => setActiveCitations(msg.citations || null)}
                            className="flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-brand-500/10 hover:bg-brand-500/20 border border-brand-500/20 text-brand-300 transition-colors cursor-pointer"
                          >
                    ...
                    {msg.role === 'assistant' && msg.related_questions && msg.related_questions.length > 0 && !isStreaming && (
                      <div className="pt-3 space-y-1.5 border-t border-gray-900/40 mt-3">
                        <span className="text-[10px] text-gray-500 font-semibold block uppercase tracking-wider">Related Questions:</span>
                        <div className="flex flex-wrap gap-2">
                          {msg.related_questions.map((q, qidx) => (
                            <button
                              key={qidx}
                              onClick={() => handleQuerySubmit(q)}
                              className="text-left text-xs bg-darkCard hover:bg-gray-800/80 border border-gray-800 text-gray-400 hover:text-white rounded-xl py-1.5 px-3.5 transition-colors cursor-pointer"
                            >
                              <span>{q}</span>
                              <ChevronRight className="h-3 w-3 inline ml-1 shrink-0 opacity-50" />
                            </button>
                          ))}
                        </div>
                      </div>
              ...
              {isStreaming && streamingResponse && (
                <div className="flex gap-4 p-4 rounded-2xl max-w-3xl border bg-darkSurface/50 border-gray-900/60 text-gray-255">
                  ...
                  <div className="space-y-1 flex-1 min-w-0">
                    <p className="text-sm leading-relaxed whitespace-pre-wrap">{streamingResponse}</p>
            ...
            {/* Input Bar panel */}
            <div className="p-4 border-t border-gray-900/60 bg-darkSurface/40">
              <form 
                onSubmit={(e) => {
                  e.preventDefault();
              ...
              >
                <input
                  type="text"
                  className="w-full bg-darkSurface border border-gray-800 text-white rounded-2xl pl-4 pr-12 py-3.5 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 text-sm placeholder-gray-500 transition-colors shadow-inner"
                ...
                <button
                  ...
                  className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-brand-500 hover:bg-brand-600 disabled:bg-gray-900 text-white disabled:text-gray-650 rounded-xl transition-all shadow"
                >
                  <Send className="h-4 w-4" />
                </button>
              </form>
              <div className="text-center text-[10px] text-gray-650 mt-2">
      ...
      {activeCitations && (
        <div className="absolute md:relative inset-y-0 right-0 w-80 bg-darkSurface border-l border-gray-900/60 z-20 flex flex-col shadow-2xl">
          <div className="p-4 border-b border-gray-900/60 flex items-center justify-between">
            ...
            <button
              ...
              className="text-gray-500 hover:text-white text-xs border border-gray-800 hover:bg-gray-800 rounded-lg py-1 px-2.5 transition-colors"
            >
              Close
            </button>
          </div>
            ...
            {activeCitations.map((citation, index) => (
              ...
              >
                <div className="flex items-center justify-between text-[10px] font-semibold text-brand-300">
                  ...
                  <span className="text-[10px] text-gray-550">Match: {Math.round((1 - citation.score) * 100)}%</span>
                </div>
                ...
                <div className="p-2.5 rounded bg-darkSurface border border-gray-900/80 text-gray-400 italic text-[11px] leading-relaxed">
                  "{citation.snippet}..."
                </div>
              </div>
            ))}
